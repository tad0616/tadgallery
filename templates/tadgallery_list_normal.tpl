    ...
    <div id="tg_container">
        <{foreach from=$photo item=pic}>
            ...
            <{if $pic.photo_edit|default:false}>
                <button href="ajax.php?op=edit_photo&sn=<{$pic.sn}>" class="btn btn-sm btn-warning fancybox fancybox.ajax editbtn" style="position:absolute;bottom:2px;right:2px;display:none;"><i class="fa fa-pencil" aria-hidden="true"></i></button>
            <{/if}>
            </div>
        <{/foreach}>
    </div>
