<{if $block.pics}>
  <{foreach from=$block.pics item=photo}>
  	<a href="<{$photo.link}>" data-photo="<{$photo.pic_url}>" style="display: block; float: left; margin: <{$block.margin}>px; text-decoration: none;" <{$photo.fancy_class}> title="<{$photo.photo_sn}>">
