<{if $block.pics|default:false}>
  ...
  $(function() {
    $('#carousel<{$block.view_csn}>').carouFredSel({
      ...
      <{if $block.staytime|default:false}>
      auto: <{$block.staytime}>,
      <{/if}>
      mouseWheel: true
    });
    ...
    <ul id="carousel<{$block.view_csn}>">
      ...
      <li>
        <a href="<{$p.link}>" data-photo="<{$p.pic_url}>" data-sn="<{$p.photo_sn}>" <{$p.fancy_class}>>
          <img src="<{$p.pic_url}>" alt="<{$p.pic_txt}>" <{if $p.photo_title|default:false}>title="<{$p.photo_title}>"<{/if}> style="<{if $p.direction=="1"}>width:<{$p.width}>px;<{else}>height:<{$p.height}>px;<{/if}>margin:0px 2px;"><span class="sr-only visually-hidden">photo-<{$p.photo_sn}></span>
        </a>
      </li>
      <{/foreach}>
    </ul>
